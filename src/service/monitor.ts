 ...
 * Provides better memory management and cleaner architecture compared to function-based approach.
 */
export class CosmosMonitor {
  ...
  private async ensureChainIdentity(): Promise<ChainIdentity | null> {
    ...
    this.chainIdentity = await cosmos.getChainIdentity(this.config.cosmosNodeRestUrl);
    if (this.chainIdentity === null) {
      logger.warn(
   ...
   */
  private async checkNodeLiveness(): Promise<{ currentHeight: number | null; shouldWait: boolean }> {
    const currentHeight = await cosmos.getBlockHeight(this.config.cosmosNodeRestUrl);
  ...
  private async handleUpgradeExecution(
    ...
    const pipelineFinishedSuccessful = await gitlab.triggerGitlabUpdatePipeline(this.config);
